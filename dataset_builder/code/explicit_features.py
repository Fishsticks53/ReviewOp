...
class ExplicitArtifacts:
    ...
    train_cardinalities: Dict[str, List[str]]
...
def _text_stats(text: str) -> Dict[str, Any]:
    ...
    sentences = max(1, clean.count(".") + clean.count("!") + clean.count("?"))
    return {
        "char_count": chars,
...
def fit_explicit_artifacts(train_frame: pd.DataFrame, numeric_columns: List[str], categorical_columns: List[str]) -> ExplicitArtifacts:
    ...
    if numeric_scaler is not None:
        numeric_values = train_frame[numeric_columns].apply(pd.to_numeric, errors="coerce")
        numeric_values = numeric_values.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    return ExplicitArtifacts(
        ...
        train_cardinalities={col: sorted(train_frame[col].fillna("unknown").astype(str).unique().tolist()) for col in categorical_columns},
    )


def build_explicit_row(row: Dict[str, Any], *, artifacts: ExplicitArtifacts, numeric_columns: List[str], categorical_columns: List[str], datetime_columns: List[str], text_column: str | None) -> Dict[str, Any]:
    explicit: Dict[str, Any] = {}

    if numeric_columns:
        numeric_values = pd.DataFrame([row]).reindex(columns=numeric_columns).apply(pd.to_numeric, errors="coerce")
        numeric_values = numeric_values.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    for column in categorical_columns:
        ...
        categories = artifacts.train_cardinalities.get(column, [])
        if len(categories) <= 50:
            for category in categories:
                explicit[f"{column}_{category}"] = int(value == category)
        else:
            explicit[f"{column}_cardinality"] = len(categories)
            explicit[f"{column}_is_known"] = int(value in categories)
    ...
    for column in datetime_columns:
        ...
        if pd.notna(parsed):
            ...
            explicit[f"{column}_days_since_epoch"] = int(parsed.timestamp() // 86400)

    if text_column:
        text = str(row.get(text_column, "")).strip()
        explicit[f"{text_column}_stats"] = _text_stats(text)
        explicit[f"{text_column}_stats"]["token_count"] = token_count(text)
