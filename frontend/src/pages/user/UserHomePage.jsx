...
export default function UserHomePage() {
  ...
  useEffect(() => {
    getProductSuggestions(token).then(setSuggestions).catch((ex) => setError(ex.message || "Failed to load suggestions"));
  }, [token]);
    ...
    <UserShell title="Discover Products">
      ...
      <form onSubmit={onSearch} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="flex gap-2">
          ...
          />
          <button className="rounded-lg bg-emerald-600 px-4 py-2 text-white">Search</button>
          <button type="button" onClick={() => nav("/create-review")} className="rounded-lg border border-emerald-600 px-4 py-2 text-emerald-700 dark:text-emerald-300">
            Create Review
          </button>
        </div>
      </form>
      ...
      <section>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {suggestions.recently_reviewed.length ? suggestions.recently_reviewed.map((p) => <ProductCard key={`recent-${p.product_id}`} product={p} />) : <p className="text-sm text-slate-600 dark:text-slate-300">No reviewed products yet.</p>}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {suggestions.similar_products.map((p) => (
            <ProductCard key={`sim-${p.product_id}`} product={p} />
          ))}
        </div>
      </section>
    </UserShell>
