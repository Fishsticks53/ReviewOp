...
export default function GraphExplorer({
  ...
  return (
    <section className="space-y-4">
      <form onSubmit={onApplyFilters} className={`rounded-[30px] border p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)] ${isDark ? "border-slate-800 bg-[#0b1220]" : "border-slate-200 bg-white"}`}>
        <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-500">Corpus Filters</p>
            <h2 className={`mt-1 text-lg font-semibold ${isDark ? "text-slate-100" : "text-slate-900"}`}>Search by domain or product ID</h2>
            <p className={`mt-1 text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              Choose a known corpus value from suggestions, then apply it to update the graph.
            </p>
          </div>
          <div className={`rounded-2xl border px-3 py-2 text-xs uppercase tracking-[0.18em] ${isDark ? "border-slate-700 text-slate-400" : "border-slate-200 text-slate-500"}`}>
            {hasActiveFilters ? "Filtered view" : "Default view"}
          </div>
        </div>
